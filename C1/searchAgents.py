...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        # daca oricare dintre colturi nu este vizitat, se returneaza fals
    ...
    def getNextState(self, state, action):
        ...
        nextx, nexty = int(x + dx), int(y + dy)
        "*** YOUR CODE HERE ***"
        # verificam daca next state intra intr-un zid
        if not self.walls[nextx][nexty]:
            visited = state[1]
            # daca next state este un corner si nu a fost vizitat, il adaugam la nodurile vizitate
            if (nextx, nexty) in self.corners and (nextx, nexty) not in visited:
                visited += ((nextx, nexty),)
            # adaugam next state-ul la lista de succesori
            children = ((nextx, nexty), visited)
            return children
        return 0
        #util.raiseNotDefined()
...
def cornersHeuristic(state, problem):
    ...
    visited = state[1]
    total = 0
    # daca colturile nu sunt vizitate le adaugam in lista de colturi
    for i in problem.corners:
        if i not in visited:
            corners.append(i)
    # cat timp exista un colt in lista de colturi
    while len(corners) > 0:
        mini = 100000
        nextCorner = currentState
        # parcurgem lista de colturi
        for i in corners:
            # calculam distanta manhattan de la starea curenta la fiecare colt
            distance = util.manhattanDistance(i, currentState)
            # cautam distanta minima
            if mini > distance:
                mini = distance
                nextCorner = i
        currentState = nextCorner
        # scoatem din lista  coltul pentru care am calculat distanta minima
        corners.remove(nextCorner)
        # adunam distanta minima la un total si il returnam
        total += mini
    return total
...
def foodHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    # lista cu locurile unde se afla mancarea in grid
    food = foodGrid.asList()
    cost = 0
    # pentru fiecare bucata de mancare din lista
    for i in food:
        # calculam distanta cu mazeDistance
        distance = (mazeDistance(position, i, problem.startingGameState))
        # cautam distanta maxima si o punem in variabila cost iar apoi o returnam
        cost = max(cost, distance)
    return cost
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        if (x, y) in self.food.asList() or len(self.food.asList()) == 0:
            return True
        return False
        #util.raiseNotDefined()
